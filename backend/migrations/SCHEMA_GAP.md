...
Дата: 2026-09-05
Джерело: тільки Git-репозиторій. Живої БД / dump-у на момент складання **немає**.
...
* **`000_initial.sql` не створений.** Відтворити 71 об'єкт «за здогадкою»
  означало б зафіксувати неправильні типи, довжини, `NULL`-політику,
  колації та дефолти. Така міграція виглядала б робочою і зламала б
  production при першому ж накаті. Файл буде створений з dump-у (§7).
...
1. Отримати `baseline_schema.sql` + інвентар view/index/FK (§7).
2. Згенерувати `000_baseline.sql` з дампу, **без ручного редагування структур**.
3. Реалізувати `schema_migrations` за дизайном у `MIGRATION_VERSIONING.md`.
4. Проштампувати існуючу production-інсталяцію як `baseline + 001..011`
   (`--stamp`, без виконання DDL).
5. Перевірити clean install на порожній MySQL у CI.
6. Поступово перенести inline DDL (§5) у версіоновані міграції.