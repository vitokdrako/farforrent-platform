"""Канонічний шар рендерингу документів: Jinja-середовище і контекст.

Причина існування: `routes/document_pdf.py` імпортував `build_document_context`,
`jinja_env`, `DOCUMENT_TEMPLATES` і `get_watermark_text` з
`routes/document_render.py` (route -> route). Тепер обидва роути залежать
від цього сервісу.

ВАЖЛИВО про `jinja_env`. У `services/doc_engine/render.py` уже існує *інше*
Jinja-середовище з іншим каталогом шаблонів і без DB-override loader.
Переносити рендер документів туди не можна — це змінило б шляхи пошуку
шаблонів і зламало наявні документи. Тому середовище перенесене сюди
без змін: той самий `TEMPLATES_DIR`, той самий `DBOverrideLoader`, той самий
`autoescape`.

`TEMPLATES_DIR` обчислюється відносно каталогу `backend/`, тому значення
збігається з попереднім (`backend/templates/documents`), хоча модуль тепер
лежить у `services/`, а не в `routes/`.
"""
...
#: `backend/templates/documents` — той самий каталог, що й до рефакторингу.
